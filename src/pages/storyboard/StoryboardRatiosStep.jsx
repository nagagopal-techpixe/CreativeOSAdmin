// pages/storyboard/StoryboardRatiosStep.jsx
import { useEffect, useState } from "react";
import {
  getStoryboardRatios,
  patchStoryboardRatio,
  toggleStoryboardRatiosSection,
} from "../../services/storyboard";
import { useStepPermissions } from "../../hooks/useStepPermissions";
import { Toggle } from "../../components/Stepsection";

export default function StoryboardRatiosStep({ userId }) {
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      ...
      setSectionBusy(true);
      try {
        await toggleStoryboardRatiosSection({ isActive: !sectionActive });
  ...
  const handleItemToggle = async (item) => {
    ...
    try {
      if (isUserMode) {
        ...
        await patchStoryboardRatio(itemId, { isActive: !item.isActive });
        setItems((prev) =>
          prev.map((t) =>
