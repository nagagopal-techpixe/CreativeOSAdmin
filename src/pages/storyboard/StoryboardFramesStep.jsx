// pages/storyboard/StoryboardFramesStep.jsx
import { useEffect, useState } from "react";
import {
  getStoryboardFrames,
  patchStoryboardFrame,
  toggleStoryboardFramesSection,
} from "../../services/storyboard";
import { useStepPermissions } from "../../hooks/useStepPermissions";
import { Toggle } from "../../components/Stepsection";

export default function StoryboardFramesStep({ userId }) {
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      ...
      setSectionBusy(true);
      try {
        await toggleStoryboardFramesSection({ isActive: !sectionActive });
  ...
  const handleItemToggle = async (item) => {
    ...
    try {
      if (isUserMode) {
        ...
        await patchStoryboardFrame(itemId, { isActive: !item.isActive });
        setItems((prev) =>
          prev.map((t) =>
