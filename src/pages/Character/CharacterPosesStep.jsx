// pages/Character/CharacterPosesStep.jsx
import { useEffect, useState } from "react";
import {
  getCharacterPoses,
  patchCharacterPose,
  toggleCharacterPosesSection,
} from "../../services/character";
import { useStepPermissions } from "../../hooks/useStepPermissions";
import { Toggle } from "../../components/Stepsection";

export default function CharacterPosesStep({ userId }) {
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      ...
      setSectionBusy(true);
      try {
        await toggleCharacterPosesSection({ isActive: !sectionActive });
  ...
  const handleItemToggle = async (item) => {
    ...
    try {
      if (isUserMode) {
        ...
        await patchCharacterPose(itemId, { isActive: !item.isActive });
        setItems((prev) =>
          prev.map((t) =>
    ...
    <div className="mt-8">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-slate-300">Pose</span>
        </div>
        <div className="flex items-center gap-2">
          {sectionBusy && (
          ...
          {displayItems.map((item) => {
            ...
            return (
              <div
                ...
                title={item.val}
                className={[
                  "relative flex flex-col items-center justify-center gap-2 w-[104px] rounded-xl border cursor-pointer select-none transition-all duration-200 py-3 px-2",
                  item.isActive
              ...
              >
                ...
                {item.dim && (
                  <span
                    className={[
                      "text-[9px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded-full leading-none",
                      item.isActive
