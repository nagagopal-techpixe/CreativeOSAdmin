...
import {
  getCharacterDetails,
  toggleCharacterDetailsSection as toggleGlobalDetailsSection,
  toggleCharacterDetailGroup,
  toggleCharacterDetailOpt,
} from "../../services/character";
import {
  fetchUserPermissions,
  toggleCharacterDetailsSection,
  toggleCharacterDetailsGroup,
  toggleCharacterDetailsOpt,
} from "../../services/users";
import { Toggle } from "../../components/Stepsection";

export default function CharacterDetailsStep({ userId }) {
  ...
  useEffect(() => {
    ...
    fetchUserPermissions(userId).then((data) => {
      const dp = data.permissions?.characterBuilder?.details ?? {
        isActive: true,
        allowedGroups: [],
        allowedOpts: {},
      };
  ...
  const isGroupAllowed = (groupId) => {
    ...
    //  undefined → allow all (initial load only)
  ...
  const displayGroups =
    isUserMode && userPerms
      ? groups.map((g) => ({
          ...g,
          isActive: g.isActive && isGroupAllowed(g._id), // ← add g.isActive &&
          opts: g.opts.map((o) => ({
            ...o,
            isActive: o.isActive && isOptAllowed(g._id, o._id), // ← add o.isActive &&
          })),
        }))
      : groups;
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      ...
      await toggleCharacterDetailsSection(userId, newActive);
    } else {
      setSectionBusy(true);
      try {
        await toggleGlobalDetailsSection({ isActive: !sectionActive });
  ...
  const handleGroupToggle = async (group) => {
    ...
    try {
      if (isUserMode) {
        ...
        await toggleCharacterDetailsGroup(userId, gId, newActive);
      } else {
        await toggleCharacterDetailGroup(gId, { isActive: !group.isActive });
        setGroups((prev) =>
          prev.map((g) =>
  ...
  const handleOptToggle = async (group, opt) => {
    ...
    try {
      if (isUserMode) {
        ...
        await toggleCharacterDetailsOpt(userId, gId, oId, newActive);
      } else {
        await toggleCharacterDetailOpt(gId, oId, { isActive: !opt.isActive });
        setGroups((prev) =>
          prev.map((g) =>
            g._id !== gId
              ? g
