// pages/image/ImageDetailsStep.jsx
import { useEffect, useState } from "react";
import {
  getImageDetails,
  toggleImageDetailsSection,
  toggleImageDetailGroup,
  toggleImageDetailOpt,
} from "../../services/image";
import {
  fetchUserPermissions,
  toggleImageDetailsSection as toggleUserImageDetailsSection,
  toggleImageDetailsGroup,
  toggleImageDetailsOpt,
} from "../../services/users";
import { Toggle } from "../../components/Stepsection";

export default function ImageDetailsStep({ userId }) {
  ...
  useEffect(() => {
    ...
    fetchUserPermissions(userId).then((data) => {
      const dp = data.permissions?.imageBuilder?.details ?? {
        isActive: true,
        allowedGroups: [],
        allowedOpts: {},
      };
  ...
  const isGroupAllowed = (groupId) => {
    ...
    return ag.includes(String(groupId));
  };
  ...
  const isOptAllowed = (groupId, optId) => {
    ...
    return ao.includes(String(optId));
  };
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      ...
      await toggleUserImageDetailsSection(userId, newActive);
    } else {
      setSectionBusy(true);
      try {
        await toggleImageDetailsSection({ isActive: !sectionActive });
  ...
  const handleGroupToggle = async (group) => {
    ...
    try {
      if (isUserMode) {
        ...
        setUserPerms((prev) => ({
          ...prev,
          allowedGroups: newAllowed,
        }));

        await toggleImageDetailsGroup(userId, gId, !isAllowed);
      } else {
        await toggleImageDetailGroup(gId, { isActive: !group.isActive });
  ...
  const handleOptToggle = async (group, opt) => {
    ...
    try {
      if (isUserMode) {
        ...
        const newAllowed = isAllowed
          ? currentAllowed.filter((id) => id !== oId) // remove
          : [...currentAllowed, oId]; // add
        ...
        await toggleImageDetailsOpt(userId, gId, oId, !isAllowed);
      } else {
        await toggleImageDetailOpt(gId, oId, { isActive: !opt.isActive });
