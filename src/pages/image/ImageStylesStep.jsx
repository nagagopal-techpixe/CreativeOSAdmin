// pages/image/ImageStylesStep.jsx
import { useEffect, useState } from "react";
import {
  getImageStyles,
  patchImageStyle,
  toggleImageStylesSection,
} from "../../services/image";
import { useStepPermissions } from "../../hooks/useStepPermissions";
import StepSection from "../../components/Stepsection";

export default function ImageStylesStep({ userId }) {
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      ...
      setSectionBusy(true);
      try {
        await toggleImageStylesSection({ isActive: !sectionActive });
  ...
  const handleItemToggle = async (item) => {
    ...
    try {
      if (isUserMode) {
        ...
        await patchImageStyle(itemId, { isActive: !item.isActive });
        setItems((prev) =>
          prev.map((t) =>
