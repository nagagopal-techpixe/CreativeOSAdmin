...
export default function ImageRatiosStep({ userId }) {
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      await userSectionToggle(displaySectionActive); //  pass current, hook negates internally
    } else {
      setSectionBusy(true);
      try {
        await toggleImageRatiosSection({ isActive: !sectionActive });
  ...
  const handleItemToggle = async (item) => {
    ...
    try {
      if (isUserMode) {
        ...
        await patchImageRatio(itemId, { isActive: !item.isActive });
        setItems((prev) =>
          prev.map((t) =>
