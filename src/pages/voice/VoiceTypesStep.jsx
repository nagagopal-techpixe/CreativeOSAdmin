...
import {
  getVoiceTypes,
  patchVoiceType,
  toggleVoiceTypesSection,
} from "../../services/voice";
import { useStepPermissions } from "../../hooks/useStepPermissions";
import StepSection from "../../components/Stepsection";

export default function VoiceTypesStep({ userId }) {
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      ...
      setSectionBusy(true);
      try {
        await toggleVoiceTypesSection({ isActive: !sectionActive });
  ...
  const handleItemToggle = async (item) => {
    ...
    try {
      if (isUserMode) {
        ...
        await patchVoiceType(itemId, { isActive: !item.isActive });
        setItems((prev) =>
          prev.map((t) =>
