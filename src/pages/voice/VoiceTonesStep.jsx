...
import {
  getVoiceTones,
  patchVoiceTone,
  toggleVoiceTonesSection,
} from "../../services/voice";
import { useStepPermissions } from "../../hooks/useStepPermissions";
import StepSection from "../../components/Stepsection";

export default function VoiceTonesStep({ userId }) {
  ...
  const { isUserMode, displayItems, displaySectionActive, userSectionToggle, userItemToggle } =
    useStepPermissions({ userId, sectionKey: "tones", globalItems: items, globalIsActive: sectionActive, builderKey: "voiceBuilder" });
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      ...
      setSectionBusy(true);
      try {
        await toggleVoiceTonesSection({ isActive: !sectionActive });
  ...
  const handleItemToggle = async (item) => {
    ...
    try {
      if (isUserMode) {
        ...
        await patchVoiceTone(itemId, { isActive: !item.isActive });
        setItems((prev) =>
