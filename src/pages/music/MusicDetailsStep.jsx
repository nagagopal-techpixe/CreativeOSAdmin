// pages/music/MusicDetailsStep.jsx
import { useEffect, useState } from "react";
import {
  getMusicDetails,
  toggleMusicDetailsSection,
  toggleMusicDetailGroup,
  toggleMusicDetailOpt,
} from "../../services/music";
import {
  fetchUserPermissions,
  toggleMusicDetailsSection as toggleUserMusicDetailsSection,
  toggleMusicDetailsGroup,
  toggleMusicDetailsOpt,
} from "../../services/users";
import { Toggle } from "../../components/Stepsection";

export default function MusicDetailsStep({ userId }) {
  ...
  useEffect(() => {
    ...
    fetchUserPermissions(userId).then((data) => {
      const dp = data.permissions?.musicBuilder?.details ?? {
        isActive: true,
        allowedGroups: [],
        allowedOpts: {},
      };
  ...
  const isGroupAllowed = (groupId) => {
    ...
    //  if undefined → allow all (initial state)
    if (!ag) return false;
  ...
  const displayGroups =
    isUserMode && userPerms
      ? groups.map((g) => ({
          ...g,
          isActive: isGroupAllowed(g._id), //  FIX
          opts: g.opts.map((o) => ({
            ...o,
            isActive: isOptAllowed(g._id, o._id), //  FIX
          })),
        }))
      : groups;
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      ...
      await toggleUserMusicDetailsSection(userId, newActive);
    } else {
      setSectionBusy(true);
      try {
        await toggleMusicDetailsSection({ isActive: !sectionActive });
  ...
  const handleGroupToggle = async (group) => {
    ...
    try {
      if (isUserMode) {
        ...
        const isCurrentlyActive = currentAllowed.includes(gId);
        const newActive = !isCurrentlyActive;
        const newAllowed = newActive
          ? [...currentAllowed, gId]
          : currentAllowed.filter((id) => id !== gId);
        setUserPerms((prev) => ({ ...prev, allowedGroups: newAllowed }));
        await toggleMusicDetailsGroup(userId, gId, newActive);
      } else {
        await toggleMusicDetailGroup(gId, { isActive: !group.isActive });
        setGroups((prev) =>
          prev.map((g) =>
  ...
  const handleOptToggle = async (group, opt) => {
    ...
    try {
      if (isUserMode) {
        ...
        const isCurrentlyActive = currentAllowed.includes(oId);
        const newActive = !isCurrentlyActive;
        const newAllowed = newActive
          ? [...currentAllowed, oId]
        ...
        await toggleMusicDetailsOpt(userId, gId, oId, newActive);
      } else {
        await toggleMusicDetailOpt(gId, oId, { isActive: !opt.isActive });
        setGroups((prev) =>
          prev.map((g) =>
            g._id !== group._id
              ? g
