// pages/music/MusicDurationsStep.jsx
import { useEffect, useState } from "react";
import {
  getMusicDurations,
  patchMusicDuration,
  toggleMusicDurationsSection,
} from "../../services/music";
import { useStepPermissions } from "../../hooks/useStepPermissions";
import { Toggle } from "../../components/Stepsection";

export default function MusicDurationsStep({ userId }) {
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      ...
      setSectionBusy(true);
      try {
        await toggleMusicDurationsSection({ isActive: !sectionActive });
  ...
  const handleItemToggle = async (item) => {
    ...
    try {
      if (isUserMode) {
        ...
        await patchMusicDuration(itemId, { isActive: !item.isActive });
        setItems((prev) =>
          prev.map((t) =>
