// pages/music/MusicTypesStep.jsx
import { useEffect, useState } from "react";
import {
  getMusicTypes,
  patchMusicType,
  toggleMusicTypesSection,
} from "../../services/music";
import { useStepPermissions } from "../../hooks/useStepPermissions";
import StepSection from "../../components/Stepsection";

export default function MusicTypesStep({ userId }) {
  ...
  const handleSectionToggle = async () => {
    if (isUserMode) {
      ...
      setSectionBusy(true);
      try {
        await toggleMusicTypesSection({ isActive: !sectionActive });
  ...
  const handleItemToggle = async (item) => {
    ...
    try {
      if (isUserMode) {
        ...
        await patchMusicType(itemId, { isActive: !item.isActive });
        setItems((prev) =>
          prev.map((t) =>
