...
import { Pencil, X, Save, ToggleLeft, ToggleRight } from "lucide-react";
...
const EditModal = ({ model, onClose, onSave }) => {
  const [form, setForm] = useState({
    ...
    isActive:   model.isActive ?? true,     // ← new
  });
  ...
  // model-level isActive toggle
  const toggleModelActive = () =>
        ...
        <div className="overflow-y-auto flex-1 p-8 space-y-7">
          ...
          <div className="flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3">
            ...
            <button
              ...
              title={form.isActive ? "Active — click to disable" : "Inactive — click to enable"}
            >
          ...
          <div className="space-y-3">
            ...
            {/* Column headers */}
            {attributes.length > 0 && (
            <div className="grid grid-cols-[1fr_1fr_1fr_80px_auto] gap-2 px-1">
  {["Key", "Type", "Value", "Max Count", "Active"].map((h) => (
    <span key={h} className="text-[9px] font-black text-zinc-600 uppercase tracking-widest">
      {h}
    </span>
  ))}
</div>
            ...
            <div className="space-y-2">
              {attributes.map((attr, idx) => (
                <div
                  key={idx}
                  className={`grid grid-cols-[1fr_1fr_1fr_80px_auto] gap-2 items-center p-3 rounded-xl border transition-colors ${
                    attr.isActive
...
{['image', 'video', 'audio'].includes(attr.dtype) ? (
  <input
    ...
    min="1"
    value={attr.maxCount ?? ""}
    onChange={(e) => updateAttribute(idx, "maxCount", e.target.value ? Number(e.target.value) : null)}
    placeholder="e.g. 2"
    className="bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-xs font-mono outline-none focus:border-zinc-500 text-white w-full"
  />
) : (
  <div />  // ← empty cell for non-media fields
)}
                  {/* isActive toggle */}
                  <button
                  ...
                  >
                    {attr.isActive ? <ToggleRight size={26} /> : <ToggleLeft size={26} />}
                  </button>
                </div>
              ))}
            </div>
